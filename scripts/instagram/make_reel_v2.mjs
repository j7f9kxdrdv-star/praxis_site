...
export function timeA(q) {
  ...
  // THIS CEILING IS 3s AND STAYS 3s, at every clip length. An earlier revision
  // let it scale with the stem whenever --seconds was passed, reasoning that a
  // longer question deserves longer reading. That was wrong, and wrong in a way
  // worth writing down, because it reads as sensible.
  //
  // `read` is NOT the time it takes to read the stem. The stem stays on screen
  // for the WHOLE clip, so reading carries on underneath the options. `read` is
  // only the delay before the REVEAL, and the reveal is the single piece of
  // motion in the clip, the thing that earns a second look from someone
  // mid-scroll. Moving it later buys no reading time at all. It spends the hook
  // on a screen that is not yet interesting, which is precisely reel #3:
  // options at 6s, average watch 2.0s, nobody reaching them.
  //
  // So a longer clip buys THINKING time, not reading time, and that time
  // belongs to the decide phase with the whole question already on screen.
  ...
  if (secondsArg) {
    // An explicit length is a target, not merely a ceiling: fill it from the
    // decide phase so --seconds 10 renders 10s rather than 9s.
    decide = Math.max(DECIDE_FLOOR, secondsArg - read);
  } else if (read + decide > MAX_TOTAL) {
