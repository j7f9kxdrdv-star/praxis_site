...
BEGIN
    ...
    (deck, 39, 'cloze',
     ...
     1),
    (deck, 40, 'cloze',
     'In the Doppler equation f′ = {{c1::f(v ± v_o)/(v ∓ v_s)}}, v is the speed of sound, v_o the observer''s speed, and v_s the source''s speed.',
     1),
    (deck, 41, 'cloze',
     'In f′ = f(v ± v_o)/(v ∓ v_s), an approaching observer takes the {{c1::+ sign (numerator)}} and an approaching source the {{c1::− sign (denominator)}}.',
     1),
    (deck, 42, 'cloze',
    ...
    (deck, 46, 'cloze',
     ...
     1),
    (deck, 47, 'cloze',
     'The beat frequency heard when two waves of slightly different frequency f₁ and f₂ interfere is {{c1::f_beat = |f₁ − f₂|}}.',
     1),
    (deck, 48, 'cloze',
